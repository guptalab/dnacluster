...
void make_edges_lexi_lower(struct TrieNode *root, string &key,int pos,bool str_path)
{
    ...
    if(st[pos]<=key.size() && key.size()<=en[pos] && dp_matrix[pos][key.size()]<=thresold)
    {
        ...
        {
            Edges.push_back({dp_matrix[pos][key.size()],{pNode->EndOfWord+1,current_read+1}});       //making it 1 based.
...
int main()
{
    ...
    string Time_taken_path = "D:\\daiict\\DNA_clusturing_New_Algorithm\\datasets\\ERR1816980\\edges_generated\\Time_taken.txt";
    ...
    string input_path = "D:\\daiict\\DNA_clusturing_New_Algorithm\\datasets\\ERR1816980\\ERR1816980_converted.txt";
        ...
        ofstream time_output(Time_taken_path, std::ios::app);
    #endif
    ...
    string output_path = "D:\\daiict\\DNA_clusturing_New_Algorithm\\datasets\\ERR1816980\\edges_generated\\thresold_" + to_string(thrr) + ".csv";
